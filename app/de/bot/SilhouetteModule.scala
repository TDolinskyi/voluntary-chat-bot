...
import de.bot.common.{CustomSecuredErrorHandler, CustomUnsecuredErrorHandler, DefaultEnv}
import de.bot.controllers.{DefaultRememberMeConfig, DefaultSilhouetteControllerComponents, RememberMeConfig, SilhouetteControllerComponents}
import de.bot.db.DBModule
import de.bot.db.common.MongoAccess
import de.bot.db.dao.MongoAuthInfoDAO
import de.bot.services.UserService
...
class SilhouetteModule extends AbstractModule with ScalaModule {
  ...
  @Provides
  def providePasswordInfoDAO(mongoAccess: MongoAccess): DelegableAuthInfoDAO[PasswordInfo] = {
    implicit lazy val formatPasswordInfo: OFormat[PasswordInfo] = Json.format[PasswordInfo]
    implicit lazy val bFormatPasswordInfo = Macros.handler[PasswordInfo]
    new MongoAuthInfoDAO[PasswordInfo](mongoAccess)
  }

  @Provides
  def provideGoogleTotpInfoDAO(mongoAccess: MongoAccess): DelegableAuthInfoDAO[GoogleTotpInfo] = {
    implicit lazy val formatPasswordInfo: OFormat[PasswordInfo] = Json.format[PasswordInfo]
    implicit lazy val bFormatPasswordInfo = Macros.handler[PasswordInfo]
    implicit lazy val formatGoogleTotpInfo: OFormat[GoogleTotpInfo] = Json.format[GoogleTotpInfo]
    implicit lazy val bFormatGoogleTotpInfo = Macros.handler[GoogleTotpInfo]

    new MongoAuthInfoDAO[GoogleTotpInfo](mongoAccess)
  }

  @Provides
  def provideOAuth1InfoDAO(mongoAccess: MongoAccess): DelegableAuthInfoDAO[OAuth1Info] = {
    implicit lazy val formatOAuth1Info = Json.format[OAuth1Info]
    implicit lazy val bFormatOAuth1Info = Macros.handler[OAuth1Info]

    new MongoAuthInfoDAO[OAuth1Info](mongoAccess)
  }

  @Provides
  def provideOAuth2InfoDAO(mongoAccess: MongoAccess): DelegableAuthInfoDAO[OAuth2Info] = {
    implicit lazy val formatOAuth2Info = Json.format[OAuth2Info]
    implicit lazy val bFormatOAuth2Info = Macros.handler[OAuth2Info]

    new MongoAuthInfoDAO[OAuth2Info](mongoAccess)
  }

  @Provides
  def provideOpenIDInfoDAO(mongoAccess: MongoAccess): DelegableAuthInfoDAO[OpenIDInfo] = {
    implicit lazy val formatOpenIDInfo = Json.format[OpenIDInfo]
    implicit lazy val bFormatOpenIDInfo = Macros.handler[OpenIDInfo]

    new MongoAuthInfoDAO[OpenIDInfo](mongoAccess)
  }
  ...
  logger.info("Silhouette module started")
}