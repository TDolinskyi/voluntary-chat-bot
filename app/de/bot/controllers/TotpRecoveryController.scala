...
import de.bot.forms.TotpRecoveryForm
...
class TotpRecoveryController @Inject() (
  ...
  def submit = UnsecuredAction.async { implicit request =>
    TotpRecoveryForm.form.bindFromRequest.fold(
      ...
      data => {
        val totpRecoveryControllerRoute = de.bot.controllers.routes.TotpRecoveryController.view(data.userID, data.sharedKey, data.rememberMe)
