...
import de.bot.forms.{TotpForm, TotpSetupForm}
...
class TotpController @Inject() (
      ...
      data => {
        totpProvider.authenticate(data.sharedKey, data.verificationCode).flatMap {
          ...
          case _: ProviderException =>
            Redirect(de.bot.controllers.routes.TotpController.view(user.userID, data.sharedKey, request.authenticator.cookieMaxAge.isDefined)).flashing("error" -> Messages("invalid.unexpected.totp"))
        }
      }
    )
  }
  ...
  def submit = UnsecuredAction.async { implicit request =>
    TotpForm.form.bindFromRequest.fold(
      ...
      data => {
        val totpControllerRoute = de.bot.controllers.routes.TotpController.view(data.userID, data.sharedKey, data.rememberMe)
